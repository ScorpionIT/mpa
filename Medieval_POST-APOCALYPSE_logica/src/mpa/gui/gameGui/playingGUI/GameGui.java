...
public class GameGui extends SimpleApplication implements AnimEventListener {
	...
	public void simpleInitApp() {
		...
		niftyHandler = new NiftyHandler(assetManager, inputManager,
				audioRenderer, guiViewPort, stateManager,
				listenerImplementation, this);
		...
		clickActionListener = new GameGuiClickListener(listenerImplementation,
				this);
		keyActionListener = new GameGuiKeyActionListener(
				listenerImplementation, niftyHandler);
	...
	protected void makeFloor(float worldDimension) {
		...
		assetManager.registerLocator(
				GameProperties.getInstance().getPath("TexturePath"),
				FileLocator.class);
	...
	public void setCamera(Vector3f newPosition) {
		...
		cam.lookAt(new Vector3f(newPosition.x, 0, newPosition.z + lz),
				new Vector3f(0, 1, 0));
	...
	private void setEventTriggers() {
		inputManager.addMapping("Shift_Map_Negative_X", new MouseAxisTrigger(
				MouseInput.AXIS_X, true));

		inputManager.addMapping("Shift_Map_Positive_X", new MouseAxisTrigger(
				MouseInput.AXIS_X, false));
		inputManager.addMapping("Shift_Map_Negative_Y", new MouseAxisTrigger(
				MouseInput.AXIS_Y, true));
		inputManager.addMapping("Shift_Map_Positive_Y", new MouseAxisTrigger(
				MouseInput.AXIS_Y, false));
		inputManager.addListener(mouseActionListener, "Shift_Map_Negative_X",
				"Shift_Map_Positive_X", "Shift_Map_Negative_Y",
				"Shift_Map_Positive_Y");

		inputManager.addMapping("Wheel_DOWN", new MouseAxisTrigger(
				MouseInput.AXIS_WHEEL, true));
		inputManager.addMapping("Wheel_UP", new MouseAxisTrigger(
		...
		inputManager.addMapping("ChooseItem", new KeyTrigger(
				KeyInput.KEY_LSHIFT));
		inputManager.addMapping("attack", new MouseButtonTrigger(1));
		inputManager.addListener(clickActionListener, "attack", "Wheel_DOWN",
				"Wheel_UP", "ChooseItem");
