...
public class World
{
	...
	public ArrayList<AbstractObject> checkForCollisionInTheRange(float xMin, float xMax, float yMin, float yMax, float collisionRay)
	{
		...
		for (AbstractObject obj : objectsInTheRange)
		{
			...
			if (MyMath.pointToLineDistance(xMin, yMin, xMax, yMax, obj.getX(), obj.getY()) - obj.getCollisionRay() - collisionRay <= 1f)
				collisions.add(obj);
		}
	...
	public ArrayList<AbstractObject> getObjectsXInTheRange(float xMin, float xMax)
	{
		...
		for (Vector2f pos : keySet)
		{
			if (((int) pos.x <= ((int) xMax) && (int) pos.x >= ((int) xMin)) || ((int) pos.y <= ((int) xMax) && (int) pos.y >= ((int) xMin)))
				abstractObjectsX.addAll(objectX.get(pos));
			else if ((((int) xMin) >= (int) pos.x && ((int) xMin) <= (int) pos.y) || (((int) xMax) >= (int) pos.x && ((int) xMax) <= (int) pos.y))
				abstractObjectsX.addAll(objectX.get(pos));
		}
	...
	public ArrayList<AbstractObject> getObjectsYInTheRange(int yMin, int yMax)
	{
		...
		for (Vector2f pos : keySet)
		{
			if (((int) pos.x <= ((int) yMax) && (int) pos.x >= ((int) yMin)) || ((int) pos.y <= ((int) yMax) && (int) pos.y >= ((int) yMin)))
				abstractObjectsY.addAll(objectY.get(pos));
			else if ((((int) yMin) >= (int) pos.x && ((int) yMin) <= (int) pos.y) || (((int) yMax) >= (int) pos.x && ((int) yMax) <= (int) pos.y))
				abstractObjectsY.addAll(objectY.get(pos));
		}
		return abstractObjectsY;
	}
