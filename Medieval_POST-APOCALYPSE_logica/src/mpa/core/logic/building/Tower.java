...
public class Tower extends AbstractPrivateProperty
{
	...
	public List<AbstractCharacter> attack()
	{
		try
		{
			...
			{
				if (p != owner && MyMath.distanceFloat(x, y, p.getX(), p.getY()) <= range + owner.getPlayerLevel().ordinal())
				{
					hitCharacters.add(p);
			...
			{
				if (m.getBoss() != owner && MyMath.distanceFloat(x, y, m.getX(), m.getY()) <= range + owner.getPlayerLevel().ordinal())
				{
					hitCharacters.add(m);
				}
			}
			...
			{
				if (t.getBoss() != owner && MyMath.distanceFloat(x, y, t.getX(), t.getY()) <= range + owner.getPlayerLevel().ordinal())
					hitCharacters.add(t);
			}
