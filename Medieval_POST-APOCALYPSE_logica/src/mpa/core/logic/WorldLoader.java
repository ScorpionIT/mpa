...
public class WorldLoader {
	...
	public int makePlayers(String playerName, World world,
			Pair<Float, Float> selectedHQ) {
		int headquarterID = 0;
		List<Pair<Float, Float>> headQuarters = mapInfo.getHeadQuarters();
		int selectedHQIndex = 0;
		for (int i = 0; i < headQuarters.size(); i++) {
			Headquarter headquarter = new Headquarter(headQuarters.get(i)
					.getFirst(), headQuarters.get(i).getSecond(), null);
			Vector2f gatheringPlace = headquarter.getGatheringPlace();

			Player player = new Player(playerName, gatheringPlace.x,
					gatheringPlace.y, 100, Level.NEWBIE, headquarter);
	...
	public void makePlayers(Map<Pair<Float, Float>, String> players, World world) {
		...
		for (int i = 0; i < headQuarters.size(); i++) {
			...
			for (Pair<Float, Float> hq : players.keySet()) {
				...
				if (hq.getFirst().equals(headQuarters.get(i).getFirst())
						&& hq.getSecond().equals(
								headQuarters.get(i).getSecond())) {
					if (players.get(hq) != null) {
						Player player = new Player(players.get(hq),
								gatheringPlace.x, gatheringPlace.y, 100,
						...
						Player player = new Player("Paola Maledetta " + i,
								gatheringPlace.x, gatheringPlace.y, 100,
								Level.NEWBIE, headquarter);
						headquarter.setOwner(player);
						world.addObject(headquarter);
